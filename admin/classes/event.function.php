...
function getAllTopViewsPremiumActiveEvents()
{
    ...
    $sql = "SELECT *, COUNT(*) FROM " . TBL . "events AS t1 LEFT JOIN " . TBL . "users AS t4 ON t1.user_id = t4.user_id INNER JOIN `" . TBL . "page_views` AS t2  ON t1.event_id = t2.event_id WHERE t1.event_status= 'Active' AND t4.user_plan != 1 AND t4.user_plan != 2 GROUP BY t1.event_id ORDER BY COUNT(*) DESC, t4.user_plan DESC LIMIT 10";
    $rs = mysqli_query($conn, $sql);
    return $rs;
...
function getAllEventsCities()
{
    ...
    $sql = "SELECT GROUP_CONCAT(city_id) as city_id FROM  " . TBL . "events WHERE event_status= 'Active' AND city_id != 0 ORDER BY city_id ASC";
