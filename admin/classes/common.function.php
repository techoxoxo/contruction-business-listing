...
# Finding Page/Ads Detail View Function #

function adpostpageview($ad_post_id)
{
    global $conn;
    $user_ip = $_SERVER['REMOTE_ADDR'];

    $check_ip = mysqli_query($conn, "SELECT * FROM " . TBL . "page_views  WHERE ad_post_id = '$ad_post_id' AND user_ip = '$user_ip' ");

    if (mysqli_num_rows($check_ip) <= 0) {
        $insertview = mysqli_query($conn, "INSERT INTO " . TBL . "page_views (`ad_post_id`, `user_ip`, `page_view_cdt`) values('$ad_post_id','$user_ip',NOW())");
...
function ad_post_pageview_count($ad_post_id)
{
    global $conn;
    $total_ad_post_view_count1 = mysqli_query($conn, "SELECT ad_post_id FROM " . TBL . "page_views WHERE ad_post_id = '$ad_post_id' ");
    $total_ad_post_view_count = mysqli_num_rows($total_ad_post_view_count1);
    return $total_ad_post_view_count;
}
